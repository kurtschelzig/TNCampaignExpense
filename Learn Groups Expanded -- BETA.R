...
DonationRecord <- read_csv("2022.csv")
...
########################
  ...
  for(i in 1:length(Starts)){
    ...
    Process$Holds[[i]] <- c( Process$Holds[[i]] ,targets,sources,Starts[i])
  }
...
Count <- 1
Row <- 1
Mean <- mean(HOLD[which(HOLD !=0)]) +sd(HOLD[which(HOLD !=0)]) 
while(REM == TRUE){

 GROUPING <- which(ResultsMatrix[,Count] > Mean & ResultsMatrix[,Count]-1 %in% Eligable )
 if(length(GROUPING > 0)){
   Eligable <- setdiff(Eligable,GROUPING-1)
   Groupings$Group[Row] <- Row
   Groupings$Members[[Row]] <- GROUPING
   Row <- Row+1
   Count <- Count +1
   ResultsMatrix[(GROUPING),] <- 0
 }
  else{Count <- Count +1}
...
forceNetwork(Links = MisLinks, Nodes = MisNodes, Source = "source",
             ...
             Group = "group", Nodesize = "size",  charge = -300, fontSize = 15,opacity = 0.6,arrows = TRUE, zoom = TRUE, opacityNoHover = 0.6)
